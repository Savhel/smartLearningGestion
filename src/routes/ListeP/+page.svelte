<script>
	...
	import { onMount } from 'svelte';
        ...
        async function AjouterEns(){
            ...
            .from('parents')
            .insert([
                { nom: nomEns, adresse: adresse, telephone: telephone },
            ])
            .select()
...
<div class="w-[80%] h-screen items-center justify-center">
    ...
    <div class="h-[20%] flex-1 flex-col">
        <form class="g-8 flex flex-col w-[400px] max-w-full mt-0 m-auto items-center justify-center">
            ...
            {/if}
            {#if InsertEns}
                <p class="text-green-500 text-sm text-center">Un nouveau parent</p>
            {/if}
            ...
            <label class="position-relative border-2 border-blue-700 rounded-lg mt-1">
                <p class={adresse ? "position absolute translate-y-[-55%] pointer-events-none text-white rounded-lg p-2 text-[0.4rem] font-semibold bg-blue-700 border-blue-300" : "position absolute translate-y-[-55%] pointer-events-none text-white rounded-lg p-2 text-[0.1rem] top-[50%] left-[6px] opacity-0"}>boite mail</p>
                <input bind:value={adresse} class="w-full rounded-md focus:outline-none text-center p-[0px] bg-transparent text-white" type="text" placeholder="Adresse">
            </label>
            <label class="position-relative border-2 border-blue-700 rounded-lg mt-1">
        ...
        <div class=" flex-col border-blue-700 w-[80%] border-2 rounded-lg mx-auto mt-4">
            {#each allEnsData as ens}
                <div class="w-full mt-2 rounded-2xl h-[24%] border-x-1 border-blue-700 items-center justify-center text-center bg-gradient-to-r via-red-500 from-green-500 to-yellow-300 flex">
                    <a href="/ListeP/{ens.id}" class="w-full h-full flex items-center justify-center"> 
